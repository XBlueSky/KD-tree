...
if __name__ == "__main__":
    ...
    with open(args.file) as csv:
        for line in csv:
            ...
            if person.get(location) == None:
                ...
                person[location] = table["["+str(ret.nearest_point[0])+","+str(ret.nearest_point[1])+"]"]
    ...
    count = 1
    # output json format
    with open('D'+args.file+'.json', 'w') as fp:
        print("{", end="\n",file=fp)
        print("'time' : "+args.file, end=",\n", file=fp)
        print("'districtAllocate' : [", end=" ", file=fp)
        for key, value in person.items():
            print("{'name' : " + key, end=",\n", file=fp)
            print("'d_id' : " + value, end="}\n", file=fp)
            if count != len(person):
                print(",\n", end="", file=fp)
            count += 1 
        print("]\n}", file=fp)
